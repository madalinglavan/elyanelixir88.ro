const categoryMeta={massage:{label:"Masaj",icon:"fa-spa",note:"Relaxare și detensionare"},fitness:{label:"Acces Fitness",icon:"fa-dumbbell",note:"Utilizare autonomă a aparatelor"},medicalPedicure:{label:"Pedichiură Medicală",icon:"fa-shoe-prints",note:"Îngrijire specializată"},nails:{label:"Îngrijirea unghiilor",icon:"fa-hand-sparkles",note:"Îngrijirea unghiilor naturale"}};
...
function updateSummary(){
  ...
  summaryItems.innerHTML=items.length?items.map(item=>`<div class="summary-item"><div><strong>${item.title}</strong><span>${item.detail} × ${item.quantity}</span></div><b>${money(item.price*item.quantity)}</b></div>`).join(""):`<div class="empty-state"><i class="fa-regular fa-heart"></i><p>Alege serviciile și urmărește aici selecția și prețul.</p></div>`;
  ...
  eligibilityMessage.innerHTML=hasFixedMembership&&qualifyingCount<3?`<i class="fa-solid fa-circle-check"></i><span>Fitness selectat: 60 Lei/lună, fără reducere. Poți crea cardul.</span>`:remainingSessions?`<i class="fa-solid fa-lock"></i><span>Mai adaugă ${remainingSessions} ${remainingSessions===1?"ședință":"ședințe"} pentru a activa reducerea Membership.</span>`:`<i class="fa-solid fa-circle-check"></i><span>Ai 15% reducere la ședințele eligibile. Fitness rămâne la prețul fix de 60 Lei.</span>`;
  ...
  const lines=["Bună ziua! Aș dori să solicit un abonament Elyan Membership cu următoarele servicii:","",`Client: ${clientName}`,`Valabilitate: ${month}`,"",...items.map(item=>`• ${item.title} — ${item.detail} × ${item.quantity}: ${money(item.price*item.quantity)}`),"",`Total servicii: ${money(subtotal)}`,`Reducere Membership (15%): − ${money(discount)}`,`TOTAL DE PLATĂ: ${money(total)}`,"","Vă rog să îmi confirmați disponibilitatea, tarifele și detaliile pentru activare. Mulțumesc!"];
...
function createMembershipCardImage(){
  ...
  ctx.fillStyle="#ffffff";ctx.font="700 54px Georgia";ctx.fillText("Serviciile alese de tine",76,224);
  ...
  ctx.fillStyle='#53685d';ctx.font='16px Arial';ctx.fillText('Salonul completează acest spațiu numai după încasarea plății.',96,validationY+61);
  ...
  ctx.fillStyle="rgba(255,255,255,.72)";ctx.font="16px Arial";ctx.fillText("Solicitare de abonament • Nu este dovadă de plată",76,height-72);
